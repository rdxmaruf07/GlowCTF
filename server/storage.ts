...
export class DatabaseStorage implements IStorage {
  ...
  // Enhanced challenge creation with additional fields
  async createEnhancedChallenge(data: any): Promise<Challenge> {
  ...
  async addChallengeFile(data: any): Promise<any> {
    const { challengeFiles } = await import("@shared/schema");
    const [file] = await db.insert(challengeFiles).values(data).returning();
    return file;
  }

  async getChallengeFiles(challengeId: number): Promise<any[]> {
    const { challengeFiles } = await import("@shared/schema");
    return await db.select().from(challengeFiles).where(eq(challengeFiles.challengeId, challengeId));
  }

  // Flag submission tracking
  async recordFlagSubmission(data: any): Promise<any> {
    const { flagSubmissions } = await import("@shared/schema");
    const [submission] = await db.insert(flagSubmissions).values(data).returning();
    return submission;
  }

  async updateFlagSubmission(userId: number, challengeId: number, flag: string, isCorrect: boolean): Promise<void> {
    const { flagSubmissions } = await import("@shared/schema");
    await db
      .update(flagSubmissions)
      .set({ isCorrect })
      .where(
        and(
          eq(flagSubmissions.userId, userId),
          eq(flagSubmissions.challengeId, challengeId),
          eq(flagSubmissions.submittedFlag, flag)
        )
      );
  }

  async getUserAttemptCount(userId: number, challengeId: number): Promise<number> {
    const { flagSubmissions } = await import("@shared/schema");
  ...
  async calculateChallengeScore(challenge: Challenge, timeToSolve: number | null, userId: number): Promise<any> {
    ...
    // First blood bonus
    const solveCount = await this.getChallengeSolveCount(challenge.id);
    if (solveCount === 0 && challenge.firstBloodBonus) {
      firstBloodBonus = challenge.firstBloodBonus;
    }

    const totalPoints = basePoints + bonusPoints + firstBloodBonus;
  ...
  async getChallengeSolveCount(challengeId: number): Promise<number> {
    const result = await db
      .select({ count: sql<number>`count(*)` })
      .from(completedChallenges)
      .where(eq(completedChallenges.challengeId, challengeId));
    return result[0]?.count || 0;
  }
  ...
  async getChallengeStats(challengeId: number): Promise<any> {
    ...
    const correctAttempts = await db
      ...
      .where(
        and(
          eq(flagSubmissions.challengeId, challengeId),
          eq(flagSubmissions.isCorrect, true)
        )
      );

    const uniqueSolvers = await db
      .select({ count: sql<number>`count(distinct ${flagSubmissions.userId})` })
      .from(flagSubmissions)
      .where(
        and(
          eq(flagSubmissions.challengeId, challengeId),
    ...
    return {
      ...
      correctAttempts: correctAttempts[0]?.count || 0,
      uniqueSolvers: uniqueSolvers[0]?.count || 0,
      successRate: totalAttempts[0]?.count > 0
        ? ((correctAttempts[0]?.count || 0) / totalAttempts[0].count * 100).toFixed(2)
        : 0
    };
  }
  ...
  async getUserChallengeProgress(userId: number, challengeId: number): Promise<any> {
    ...
    const attempts = await db
      ...
      .orderBy(flagSubmissions.submittedAt);

    return {
      attemptCount: attempts.length,
      lastAttempt: attempts[attempts.length - 1]?.submittedAt || null,
      hasCorrectSubmission: attempts.some(a => a.isCorrect)
    };
  }
