...
import { ExternalLink, Flag, Clock, Award, BookOpen, Download, Terminal } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { apiRequest } from "@/lib/queryClient";

interface PlatformCTFChallenge {
  id: string;
  title: string;
  category: string;
  difficulty: string;
...
export default function PlatformCTFChallengeList() {
  const { toast } = useToast();
  const [selectedChallenge, setSelectedChallenge] = useState<PlatformCTFChallenge | null>(null);
  ...
  // Fetch Platform CTF challenges
  const { data: challenges = [], isLoading, error } = useQuery({
    queryKey: ["/api/platformctf/challenges"],
  });
  
  // Filter challenges by difficulty
  const filteredChallenges = challenges.filter(
    (challenge: PlatformCTFChallenge) => challenge.difficulty === activeTab
  );

  // Handle challenge selection
  const handleChallengeSelect = (challenge: PlatformCTFChallenge) => {
  ...
  const handleSubmitFlag = async () => {
    ...
    try {
      ...
      const response = await apiRequest("POST", "/api/platformctf/submit", {
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={selectedChallenge?.flag_format || "flag{...}"}
              />
            </div>
