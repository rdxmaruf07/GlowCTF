        ...
        >
          ...
          >
            <motion.div variants={slideInVariants}>
              <h1 className="text-2xl md:text-3xl font-semibold tracking-tight mb-2">Dashboard</h1>
              <p className="text-muted-foreground">
                Welcome back, <span className="text-primary font-medium">{user?.username}</span>. Ready to hack?
              </p>
            </motion.div>

            {!statsLoading && userStats && (
              <motion.div
                className="mt-4 md:mt-0 flex items-center bg-card px-4 py-3 rounded-lg border border-border hover:border-primary/50 transition-all duration-200 hover-lift accent-border-yellow"
                variants={slideInRightVariants}
              >
                <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-br from-orange-400 to-red-500 mr-3">
                  <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-white"><path d="M17.55 20.1c-.93.45-2.2.15-2.74-.37A3.93 3.93 0 0 0 13 19c-1.86 0-3.3.91-3.8 1.32-.55.52-1.81.82-2.74.37a1.12 1.12 0 0 1-.38-1.56c.83-1.17 2.03-3.18 2.03-6.13 0-.22-.3-1.78 2.89-3.6 1.03-.58 1.81-1.54 1.81-3.4H12c-3.86 0-5 2.45-5 4.12 0 .24.01.47.04.69" /><path d="M14.48 14.95a1 1 0 1 0-1.9 0M16 14a1 1 0 1 0 0-2" /><path d="M11.25 17a.25.25 0 1 0 .25.25.25.25 0 0 0-.25-.25"/><path d="M16 18a1 1 0 1 0 0-2" /><path d="M8 10a1 1 0 1 0 0-2" /><path d="M11.97 2h.06c1.4 0 2.4 0 3.05.1.67.08 1.22.57 1.4 1.23.2.74.13 1.67.05 2.8-.05.6-.67 1.1-1.4 1.1H8.87c-.73 0-1.35-.5-1.4-1.1-.08-1.13-.15-2.06.05-2.8.18-.66.73-1.15 1.4-1.23a17.18 17.18 0 0 1 3.05-.1Z" /></svg>
                </div>
                <div>
                  <span className="text-foreground font-medium block text-sm">Current Streak</span>
                  <span className="text-orange-500 font-bold text-lg">{userStats.streak} days</span>
                </div>
              </motion.div>
            )}
